...
def get_todos_from_category(category) -> List[Todo]:
    c.execute('select * from todos where category = :category', {'category': category})
    results = c.fetchall()
    todos = []
    for result in results:
        todos.append(Todo(*result))
